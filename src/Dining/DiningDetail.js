...
import { DECIMAL_RADIX, diningNamespace, imageGallery, HeavyOrange, LightBlueButtonBackground, ExtraHeavyBlueGreen } from '../Constants';
                ...
                {status === 200 && (
                    <div style={{flex: 1}}>
                        ...
                        <div style={{height: "25%", display: "flex"}}>
                            <div style={{flexBasis: "50%", backgroundImage: `url(${restaurant.logo})`, backgroundSize: 'cover', backgroundPosition: 'center'}} />
                            <div style={{flexBasis: "50%", backgroundColor: LightBlueButtonBackground}}>
                                <div style={{height: "50%", display: "flex", flexDirection: "column", justifyContent: "flex-end", alignItems: "center", paddingBottom: 20}}>
                                    {restaurant.phone && <div>CALL TODAY: {restaurant.phone}</div>}
                                ...
                                {restaurant.mapRestaurant.length > 0 && 
                                    <div style={{height: "50%", paddingLeft: 20, paddingRight: 20}}>
                                        <div style={{backgroundColor: HeavyOrange, width: "100%", height: "50%", display: "flex", justifyContent: "center", alignItems: "center"}}>
                                            {restaurant.mapRestaurant[0].title.toUpperCase()} MAP
                                        </div>
                                    </div>
                                }
                            </div>
                        </div>
                        <div style={{height: "25%", display: "flex", backgroundColor: ExtraHeavyBlueGreen}}>
                            <div style={{flexBasis: "50%", paddingTop: 20, paddingLeft: 20, paddingRight: 20}}>{restaurant.description}</div>
                            <div style={{flexBasis: "50%", paddingTop: 20, paddingLeft: 20, paddingRight: 20}}>
                                <div>CUISINE: {restaurant.guide.cuisine}</div>
                                <div style={{display: "flex", justifyContent: "space-between"}}>
                                    <span>TAKEAWAY: {restaurant.guide.takeaway}</span>
                                    <span>WIFI: {restaurant.guide.wifi}</span>
                                    <span>PARKING: {restaurant.guide.parking}</span>
                                </div>
                                <div>COURTESY TRANSPORT: {restaurant.guide.courtesy}</div>
                                <div>CARDS ACCEPTED: {restaurant.guide.cards}</div>
                                <div>PRICE GUIDE:<br />{restaurant.guide.price}</div>
                            </div>
                        </div>
                    </div>
                )}
