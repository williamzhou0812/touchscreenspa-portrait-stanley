                ...
                <div style={{width: "100%", height: "100%", display: "flex"}}>
                    ...
                    {(!!destination && status === 200) && (
                        <div style={{width: "86%", height: "100%"}}>
                            ...
                            <div style={{height: "50%", backgroundColor: ExtraHeavyBlueGreen, paddingLeft: 20, color: "white"}}>
                                <div style={{height: "30%", letterSpacing: "5px", display: "flex", alignItems: "center", fontSize: "28pt"}}>{destination.title}</div>
                                <div style={{height: "70%", display: "flex"}}>
                                    <div style={{width: "50%", height: "100%"}}>
                                        {destination.description}
                                    </div>
                                    <div style={{width: "50%", height: "100%", marginLeft: 10}}>
                                        <div>PROVINCE: {destination.province}</div>
                                        <div>CLOSEST AIRPORT: {destination.airport}</div>
                                        <div onClick={this.goToExplore} style={{height: "28%", width: "80%", backgroundColor: HeavyOrange, marginLeft: 10, ...this.styles.horizontalVerticalCenter}}>EXPLORE</div>
                                        {destination.mapDestination.length > 0 && (
                                            <div style={{height: "17%", width: "80%", marginLeft: 10, backgroundColor: LightBlueButtonBackground, ...this.styles.horizontalVerticalCenter}}>{destination.mapDestination[0].title}</div>
                                        )}
                                        {destination.mapDestination.length > 1 && (
                                            <div style={{height: "17%", width: "80%", marginLeft: 10, backgroundColor: LightBlueButtonBackground, ...this.styles.horizontalVerticalCenter}}>{destination.mapDestination[1].title}</div>
                                        )}
                                    </div>
                                </div>
                            </div>
